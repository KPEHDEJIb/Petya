...
def bhelp(update: Update, context: CallbackContext):
    ...
    if user_id in players_info['starting_players']:
        if update.message.chat.type == 'private':
            if players_info['starting_players'][user_id] == 'choosing':
                update.message.reply_text('Нажми на кнопку с названием игры, в которую хочешь поиграть.')
            elif players_info['starting_players'][user_id] == 'viewscore':
                update.message.reply_text('Нажми на кнопку с названием игры, статистику которой хочешь узнать.')
        ...
        if update.message.chat.type == 'private':
            update.message.reply_text('*тут будут расписаны правила игры в "5"*')

        else:
            update.message.reply_text('Игра начата! Пиши мне в личные сообщения, пожалуйста.')

    elif user_id in players_info['players_in_play']['FofD']:
        if update.message.chat.type == 'private':
            update.message.reply_text('*тут будут расписаны правила игры в "Поле Чудес"*')

        else:
            update.message.reply_text('Игра начата! Пиши мне в личные сообщения, пожалуйста.')

    else:
        update.message.reply_text('Просто напиши /start мне в личные сообщения, выбери интересующую тебя игру и я начну'
                                  ' создание сессии.')
...
def main():
    ...
    dp.add_handler(CommandHandler('adstop', admin_stop))
